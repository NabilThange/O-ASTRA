...
export class AgentProcessor {
  ...
  constructor(
    ...
    this.services = {
      ...
      'openai-compatible': this.openaiCompatibleService,
  ...
  handleTaskMessage({ taskId }: { taskId: string }) {
    this.logger.log(`Task message event received for task ID: ${taskId}`);
  ...
  private async runIteration(taskId: string): Promise<void> {
    ...
    try {
      ...
      // If no tool results were generated (e.g. model responded with text/thinking only),
      // the assistant turn is complete. Wait for subsequent user message.
      if (generatedToolResults.length === 0) {
        this.logger.debug(
          `No tool calls generated for task ID: ${taskId}. Turn complete; waiting for user input.`,
        );
        this.isProcessing = false;
        this.currentTaskId = null;
        return;
